...
public:
    ...
    virtual double Evaluate(const FormulaAST::CellLookup& lookup) const = 0;
...
public:
    ...
    double Evaluate(const FormulaAST::CellLookup& lookup) const override {
        double left  = lhs_->Evaluate(lookup);
        double right = rhs_->Evaluate(lookup);
        double result = 0.0;
        switch (type_) {
...
public:
    ...
    double Evaluate(const FormulaAST::CellLookup& lookup) const override {
        double val = operand_->Evaluate(lookup);
...
public:
    ...
    double Evaluate(const FormulaAST::CellLookup&) const override { return value_; }
...
public:
    ...
    void exitCell(FormulaParser::CellContext* ctx) override {
        ...
        args_.push_back(std::make_unique<CellExpr>(&cells_.front()));
    }
